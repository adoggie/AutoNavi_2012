package com.sw2us.newgis
{
	...
	public class AoDataQueue
	{
		...
		public function pushData(module:int,data:Object):void{
			//var x:ArrayUtil;
			...
			if( _cachedpolicy == CachedPolicy_Size){
				...
				for(var n:int=ar.length-1;n>=0;n--){					
					var d:Object = ar[n];
		...
		public function getModuleDataByTime(module:int,tick:int):Object{
			...
			var datalist:Array = _modules[module]  as Array; //模块数据列表
			...
			for(;start<datalist.length;start++){
				d = datalist[start];
				//trace(d.time,tick);
				if( d.time >tick){ //must be  '>'
					_lastpos = start;
					return d;
				}
			}			
