...
package com.sw2us.newgis.map
{
	...
	public class MapAo 
	{
		...
		private var _gpsdata:Object = null;
		...
		{			
			...
			_marker = MapAoMarker.create(this,layer,AppResource.instance().getAoBitmapByAngle(180),loc );
		...
		//是否显示轨迹
		public function showPath(v:Boolean):void{
			getPath().setVisible(v);
		}
		
		//gps位置改变传送到此，改变在地图上的位置 
		public function setGpsData(d:Object):void{
		...
		public function moveCenter():void{
			...
			loc = _ao.getLocation();
			/*
			if( this._gpsdata == null){
				return ;
			}
			loc = new Location(_gpsdata.lon,_gpsdata.lat);
			*/
			if(loc.lat==0 || loc.lon == 0){
				return;
			}
			_layer.map.moveTo(loc);
		}
